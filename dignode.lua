...
minetest.register_on_placenode(function(pos, node, placer)
    ...
    if np then
      -- apply dust
      local air_node = minetest.get_node(np)
      ...
      local pressure = cleanroom.get_pressure(np)
      local node_name, node_param2 = cleanroom.get_pressure_node(pressure)
      if (air_node.name~=node_name) or (air_node.param2~=node_param2) then
        minetest.swap_node(np, {name=node_name, param2=node_param2})
        local timer = minetest.get_node_timer(np)
